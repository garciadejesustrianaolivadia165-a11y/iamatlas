...
const WEEK_COLS_S = ["L","M","X","J","V","S","D"];
    ...
    <div style={{ background: "white", overflowY: "auto", height: "100%" }}>
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: "6px" }}>
          {DAY_HEADERS.map(d => <div key={d} style={{ textAlign: "center", fontSize: "11px", color: "#aaa", fontWeight: "600", padding: "4px0" }}>{d}</div>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
          {miniDays.map((day, i) => {
            const isToday = day === today.getDate() && miniMonth === today.getMonth() && miniYear === today.getFullYear();
            return <div key={i} onClick={() => { if (day && isMobile) setSidebarOpen(false); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "30px", height: "30px", margin: "1px auto", borderRadius: "50%", fontSize: "12px", cursor: day ? "pointer" : "default", background: isToday ? "#78C609" : "transparent", color: !day ? "transparent" : isToday ? "white" : "#333", fontWeight: isToday ? "700" : "400" }}>{day ?? ""}</div>;
          })}
        </div>
      </div>
    ...
    <div style={{ display: "flex", height: "calc(100vh - 72px)", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "6px" : "10px", padding: isMobile ? "10px 12px" : "12px 20px", borderBottom: "1px solid #eee", background: "white", flexShrink: 0 }}>
          ...
          <button onClick={prevView} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#555", padding: "2px 4px", flexShrink: 0 }}>‹</button>
          <span style={{ fontSize: isMobile ? "12px" : "15px", fontWeight: "600", color: "#343C6A", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {isMobile ? `${MONTHS_ES[viewMonth].slice(0,3)} ${viewYear}` : `${MONTHS_ES[viewMonth]} ${viewYear}`}
          </span>
          <button onClick={nextView} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#555", padding: "2px 4px", flexShrink: 0 }}>›</button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", background: "white", borderBottom: "1px solid #eee", flexShrink: 0 }}>
          {(isMobile ? WEEK_COLS_S : WEEK_COLS).map((d, i) => (
            <div key={d} style={{ textAlign: "center", padding: isMobile ? "6px 0" : "10px 0", fontSize: isMobile ? "11px" : "13px", fontWeight: "600", color: i >= 5 ? "#DA007C" : "#888", borderLeft: i > 0 ? "1px solid #eee" : "none" }}>{d}</div>
          ))}
        </div>

        {/* Grid mensual */}
        <div style={{ flex: 1, overflowY: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gridAutoRows: `minmax(${isMobile ? 60 : 110}px, 1fr)` }}>
            {calDays.map((day, i) => {
              const isToday = day === today.getDate() && viewMonth === today.getMonth() && viewYear === today.getFullYear();
              const isWeekend = i % 7 >= 5;
              const events  = day ? (monthEvents[day] ?? []) : [];
              const visible = events.slice(0, isMobile ? 1 : 2);
              const extra   = events.length - visible.length;
              return (
                <div key={i} style={{ borderLeft: i % 7 !== 0 ? "1px solid #eee" : "none", borderBottom: "1px solid #eee", background: !day ? "#fafafa" : isWeekend ? "#fdf8ff" : "white", padding: isMobile ? "3px" : "6px" }}>
                  {day && (
                    <>
                      <div style={{ width: isMobile ? "20px" : "26px", height: isMobile ? "20px" : "26px", borderRadius: "50%", background: isToday ? "#78C609" : "transparent", color: isToday ? "white" : isWeekend ? "#DA007C" : "#333", fontSize: isMobile ? "10px" : "13px", fontWeight: isToday ? "700" : "500", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "2px" }}>{day}</div>
                      {visible.map((ev, ei) => (
                        <div key={ei} onClick={() => setSelectedEvent({ ...ev, day })}
                          style={{ background: ev.bg, color: ev.textColor, fontSize: isMobile ? "9px" : "11px", fontWeight: "600", borderRadius: "4px", padding: isMobile ? "1px 3px" : "2px 6px", marginBottom: "2px", cursor: "pointer", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}
                          onMouseEnter={e => { e.currentTarget.style.filter = "brightness(0.93)"; }}
                          onMouseLeave={e => { e.currentTarget.style.filter = "brightness(1)"; }}
                        >{isMobile ? "●" : ev.label}</div>
                      ))}
                      {extra > 0 && <div style={{ fontSize: "9px", color: "#aaa", paddingLeft: "2px" }}>+{extra}</div>}
                    </>
                  )}
                </div>
              );
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: isMobile ? "20px 20px 0 0" : "16px", width: isMobile ? "100%" : "340px", maxWidth: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.15)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
              {[
                ...
                { label: "Fecha",    value: `${selectedEvent.day} ${MONTHS_ES[viewMonth]} ${viewYear}` },
