...
const COL_DAYS_SHORT = ["L","M","X","J","V","S","D"];
...
export default function CalendarIndex() {
      ...
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
          {calDays.map((day, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "30px", height: "30px", margin: "1px auto", borderRadius: "50%", fontSize: "12px", cursor: day ? "pointer" : "default", background: isToday ? "#78C609" : "transparent", color: !day ? "transparent" : isToday ? "white" : "#333", fontWeight: isToday ? "700" : "400" }}>{day ?? ""}</div>
            );
          })}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 72px)", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
      ...
      {!isMobile && sidebarOpen && (
        <div style={{ width: "270px", borderRight: "1px solid #eee", flexShrink: 0, overflowY: "auto" }}>
          <SidebarContent />
        </div>
      )}
      {isMobile && sidebarOpen && (
        <>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: "270px", zIndex: 201, boxShadow: "2px 0 16px rgba(0,0,0,0.15)", overflowY: "auto" }}>
            <SidebarContent />
          </div>
        </>
      )}
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "6px" : "10px", padding: isMobile ? "10px 12px" : "12px 20px", borderBottom: "1px solid #eee", background: "white", flexShrink: 0 }}>
          ...
          {/* ← FIX: value explícito en options, minWidth en select */}
          <div style={{ position: "relative", flexShrink: 0 }}>
            <select
              value="Semanal"
              onChange={e => {
                if (e.target.value === "Mensual") navigate("month", { relative: "path" });
                if (e.target.value === "Diario")  navigate("week",  { relative: "path" });
              }}
          ...
          {/* ← FIX: flexShrink: 0 para que el botón siempre sea visible */}
        ...
        <div style={{ background: "white", flexShrink: 0, borderBottom: "1px solid #eee" }}>
          <div style={{ display: "flex" }}>
            ...
            {(isMobile ? COL_DAYS_SHORT : COL_DAYS).map((day, i) => (
              <div key={day} style={{ flex: 1, textAlign: "center", height: "38px", lineHeight: "38px", fontSize: isMobile ? "11px" : "13px", fontWeight: "600", color: i >= 5 ? "#DA007C" : "#888", borderLeft: "1px solid #eee", background: hoveredDay === i ? "#e8e8e8" : i >= 5 ? "#f8f8f8" : "white", transition: "background 0.15s ease" }}>{day}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          <div style={{ display: "flex", position: "relative" }}>
            <div style={{ width: isMobile ? "44px" : "64px", flexShrink: 0 }}>
              {hours.map(h => (
                <div key={h} style={{ height: `${ROW_H}px`, padding: isMobile ? "6px 4px 0" : "6px 8px 0", fontSize: isMobile ? "9px" : "11px", color: "#bbb", borderBottom: "1px solid #f0f0f0", boxSizing: "border-box" }}>{h}</div>
              ))}
            </div>
            {COL_DAYS.map((day, dayIdx) => (
              <div key={day}
                onMouseEnter={() => !isMobile && setHoveredDay(dayIdx)}
                onMouseLeave={() => !isMobile && setHoveredDay(null)}
                style={{ flex: 1, position: "relative", borderLeft: "1px solid #eee", background: hoveredDay === dayIdx ? (dayIdx >= 5 ? "#ebebeb" : "#f0f0f0") : (dayIdx >= 5 ? "#f8f8f8" : "white"), transition: "background 0.15s ease" }}
              >
                ...
                {sampleEvents.filter(ev => ev.day === dayIdx).map((ev, i) => (
                  <div key={i} onClick={() => setSelectedEvent(ev)}
                    style={{ position: "absolute", top: `${ev.startHour * ROW_H}px`, left: "2px", right: "2px", height: `${ev.duration * ROW_H - 3}px`, background: ev.bg, borderRadius: "6px", padding: "3px 6px", overflow: "hidden", display: "flex", alignItems: "center", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}
                    onMouseEnter={e => { e.currentTarget.style.filter = "brightness(0.93)"; }}
                    onMouseLeave={e => { e.currentTarget.style.filter = "brightness(1)"; }}
                  >
                    <span style={{ fontSize: isMobile ? "9px" : "11px", fontWeight: "600", color: ev.textColor, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{ev.label}</span>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: isMobile ? "20px 20px 0 0" : "16px", width: isMobile ? "100%" : "340px", maxWidth: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.15)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div><p style={{ fontSize: "11px", color: "#aaa", margin: "0 0 2px" }}>Horario</p><p style={{ fontSize: "14px", color: "#333", margin: 0, fontWeight: "500" }}>{startHourLabel(selectedEvent.startHour)} – {endHourLabel(selectedEvent.startHour, selectedEvent.duration)}</p></div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div><p style={{ fontSize: "11px", color: "#aaa", margin: "0 0 2px" }}>Día</p><p style={{ fontSize: "14px", color: "#333", margin: 0, fontWeight: "500" }}>{COL_DAYS[selectedEvent.day]}</p></div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div><p style={{ fontSize: "11px", color: "#aaa", margin: "0 0 2px" }}>Duración</p><p style={{ fontSize: "14px", color: "#333", margin: 0, fontWeight: "500" }}>{selectedEvent.duration >= 1 ? `${selectedEvent.duration}h` : `${Math.round(selectedEvent.duration * 60)} min`}</p></div>
              </div>
            </div>
        ...
        <div onClick={() => setModalOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 1000 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: isMobile ? "20px 20px 0 0" : "20px", padding: isMobile ? "24px 20px 32px" : "32px", width: isMobile ? "100%" : "540px", maxWidth: isMobile ? "100%" : "90vw", boxShadow: "0 8px 40px rgba(0,0,0,0.15)", maxHeight: isMobile ? "90vh" : "auto", overflowY: isMobile ? "auto" : "visible" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
              ...
              {!isMobile && <div style={{ textAlign: "right" }}><p style={{ fontSize: "12px", color: "#aaa", margin: "0 0 2px" }}>Registro</p><p style={{ fontSize: "13px", fontWeight: "600", color: "#333", margin: 0 }}>{today.toLocaleDateString("es-ES", { day: "2-digit", month: "long", year: "numeric" })}</p></div>}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: "12px", marginTop: "28px" }}>
              ...
              <button onClick={() => setModalOpen(false)} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", padding: "11px 22px", borderRadius: "100px", border: "2px solid #DA007C", background: "white", color: "#DA007C", fontSize: "14px", fontWeight: "600", cursor: "pointer", width: isMobile ? "100%" : "auto" }} onMouseEnter={e => { e.currentTarget.style.background = "#FFF0F8"; }} onMouseLeave={e => { e.currentTarget.style.background = "white"; }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
