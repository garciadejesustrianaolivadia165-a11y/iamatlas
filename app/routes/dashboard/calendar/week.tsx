...
const inputStyle: React.CSSProperties = { width:"100%", padding:"12px 18px", borderRadius:"100px", border:"1.5px solid #e0e0e0", fontSize:"14px", color:"#333", outline:"none", boxSizing:"border-box", background:"white" };
const selectStyle: React.CSSProperties = { padding:"5px 28px 5px 14px", borderRadius:"100px", border:"1.5px solid #78C609", background:"white", color:"#78C609", fontSize:"13px", fontWeight:"600", cursor:"pointer", appearance:"none", outline:"none" };
...
export default function CalendarDay() {
    ...
    <div style={{ display:"flex", height:"calc(100vh - 72px)", fontFamily:"Inter, sans-serif", overflow:"hidden" }}>
          ...
          <div style={{ padding:"20px 20px 0" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7, 1fr)" }}>
              {miniDays.map((day, i) => {
                ...
                return <div key={i} onClick={() => { if (day) setCurrentDate(new Date(miniYear, miniMonth, day)); }} style={{ display:"flex", alignItems:"center", justifyContent:"center", width:"30px", height:"30px", margin:"1px auto", borderRadius:"50%", fontSize:"12px", cursor: day ? "pointer" : "default", background: isTod ? "#78C609" : "transparent", color: !day ? "transparent" : isTod ? "white" : "#333", fontWeight: isTod ? "700" : "400" }}>{day ?? ""}</div>;
              })}
            </div>
          </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"14px", padding:"14px 24px", borderBottom:"1px solid #eee", background:"white", flexShrink:0 }}>
          ...
          <button onClick={prevDay} style={{ background:"none", border:"none", cursor:"pointer", fontSize:"18px", color:"#555", padding:"2px 6px" }}>‹</button>
          <span style={{ fontSize:"15px", fontWeight:"600", color:"#343C6A", minWidth:"280px" }}>{dayLabel}</span>
          <button onClick={nextDay} style={{ background:"none", border:"none", cursor:"pointer", fontSize:"18px", color:"#555", padding:"2px 6px" }}>›</button>
          {isToday && <span style={{ fontSize:"11px", fontWeight:"700", background:"#78C609", color:"white", padding:"3px 10px", borderRadius:"100px" }}>Hoy</span>}
          <div style={{ position:"relative" }}>
            <select value="Diario" onChange={e => {
  ...
  if (e.target.value === "Mensual") navigate("/calendar/month");
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"14px" }}>
            ...
            <button onClick={() => setModalOpen(true)} style={{ padding:"8px 20px", borderRadius:"100px", border:"2px solid #DA007C", background:"white", color:"#DA007C", fontSize:"13px", fontWeight:"600", cursor:"pointer" }}
              onMouseEnter={e=>{ e.currentTarget.style.background="#FFF0F8"; }} onMouseLeave={e=>{ e.currentTarget.style.background="white"; }}>Agregar un evento</button>
          </div>
        </div>
        ...
        <div style={{ background:"white", flexShrink:0, borderBottom:"1px solid #eee" }}>
          <div style={{ display:"flex" }}>
            ...
            <div style={{ flex:1, textAlign:"center", height:"54px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", borderLeft:"1px solid #eee" }}>
              <span style={{ fontSize:"11px", color:"#aaa", fontWeight:"600", textTransform:"uppercase", letterSpacing:"0.5px" }}>{DAYS_ES[currentDate.getDay()]}</span>
              <div style={{ width:"34px", height:"34px", borderRadius:"50%", marginTop:"3px", background: isToday ? "#78C609" : "transparent", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <span style={{ fontSize:"18px", fontWeight:"700", color: isToday ? "white" : "#333" }}>{currentDate.getDate()}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:"auto" }}>
          <div style={{ display:"flex", position:"relative" }}>
            <div style={{ width:"64px", flexShrink:0 }}>
              {hours.map(h => <div key={h} style={{ height:`${ROW_H}px`, padding:"6px 8px 0", fontSize:"11px", color:"#bbb", borderBottom:"1px solid #f0f0f0", boxSizing:"border-box" }}>{h}</div>)}
            </div>
            <div style={{ flex:1, position:"relative", borderLeft:"1px solid #eee", background:"white" }}>
              ...
              {isToday && (
                <div style={{ position:"absolute", top:`${nowTop}px`, left:0, right:0, display:"flex", alignItems:"center", zIndex:10, pointerEvents:"none" }}>
                  <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#DA007C", flexShrink:0, marginLeft:"-5px" }} />
                  <div style={{ flex:1, height:"2px", background:"#DA007C" }} />
                </div>
              )}
              {dayEventsData.map((ev, i) => (
                <div key={i} onClick={() => setSelectedEvent(ev)}
                  style={{ position:"absolute", top:`${ev.startHour*ROW_H}px`, left:"8px", right:"8px", height:`${ev.duration*ROW_H-4}px`, background:ev.bg, borderRadius:"10px", padding:"10px 16px", overflow:"hidden", cursor:"pointer", boxShadow:"0 2px 6px rgba(0,0,0,0.08)", borderLeft:`4px solid ${ev.textColor}`, display:"flex", flexDirection:"column", justifyContent:"center" }}
                  onMouseEnter={e=>{ e.currentTarget.style.filter="brightness(0.93)"; }} onMouseLeave={e=>{ e.currentTarget.style.filter="brightness(1)"; }}
                >
                  <span style={{ fontSize:"13px", fontWeight:"700", color:ev.textColor }}>{ev.label}</span>
                  <span style={{ fontSize:"11px", color:ev.textColor, opacity:0.8, marginTop:"3px" }}>{startHourLabel(ev.startHour)} – {endHourLabel(ev.startHour, ev.duration)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div onClick={e=>e.stopPropagation()} style={{ background:"white", borderRadius:"16px", width:"340px", maxWidth:"90vw", boxShadow:"0 8px 32px rgba(0,0,0,0.15)", overflow:"hidden" }}>
            ...
            <div style={{ padding:"20px 24px", display:"flex", flexDirection:"column", gap:"14px" }}>
              {[
                ...
                { icon:<><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></>, label:"Fecha", value:dayLabel },
                ...
                <div key={row.label} style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                  ...
                  <div><p style={{ fontSize:"11px", color:"#aaa", margin:"0 0 2px" }}>{row.label}</p><p style={{ fontSize:"14px", color:"#333", margin:0, fontWeight:"500" }}>{row.value}</p></div>
                </div>
              ))}
            </div>
            <div style={{ padding:"0 24px 20px" }}>
              <button onClick={() => setSelectedEvent(null)} style={{ width:"100%", padding:"10px", borderRadius:"100px", border:"2px solid #DA007C", background:"white", color:"#DA007C", fontSize:"13px", fontWeight:"600", cursor:"pointer" }}
                onMouseEnter={e=>{ e.currentTarget.style.background="#FFF0F8"; }} onMouseLeave={e=>{ e.currentTarget.style.background="white"; }}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div onClick={() => setModalOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.35)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"white", borderRadius:"20px", padding:"32px", width:"540px", maxWidth:"90vw", boxShadow:"0 8px 40px rgba(0,0,0,0.15)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"24px" }}>
              <div><p style={{ fontSize:"17px", fontWeight:"700", color:"#1a1a1a", margin:"0 0 4px" }}>Agregar evento</p><p style={{ fontSize:"13px", color:"#888", margin:0 }}>Completa la información del nuevo evento</p></div>
              <div style={{ textAlign:"right" }}><p style={{ fontSize:"12px", color:"#aaa", margin:"0 0 2px" }}>Registro</p><p style={{ fontSize:"13px", fontWeight:"600", color:"#333", margin:0 }}>{today.toLocaleDateString("es-ES",{ day:"2-digit", month:"long", year:"numeric" })}</p></div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
              <div><label style={{ fontSize:"13px", color:"#555", fontWeight:"500", display:"block", marginBottom:"8px" }}>Descripción del evento</label><input type="text" placeholder="Ej: Práctica de Pádel" value={evDesc} onChange={e=>setEvDesc(e.target.value)} style={inputStyle} /></div>
              ...
              {!allDay && (<div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
                <div><label style={{ fontSize:"13px", color:"#555", fontWeight:"500", display:"block", marginBottom:"8px" }}>Hora de inicio</label><input type="time" value={evTimeStart} onChange={e=>setEvTimeStart(e.target.value)} style={inputStyle} /></div>
                <div><label style={{ fontSize:"13px", color:"#555", fontWeight:"500", display:"block", marginBottom:"8px" }}>Hora de fin</label><input type="time" value={evTimeEnd} onChange={e=>setEvTimeEnd(e.target.value)} style={inputStyle} /></div>
              </div>)}
              ...
              {includeOthers && (<div><label style={{ fontSize:"13px", color:"#555", fontWeight:"500", display:"block", marginBottom:"8px" }}>Correo de invitados</label><input type="email" placeholder="invitado@gmail.com" value={guestEmail} onChange={e=>setGuestEmail(e.target.value)} style={inputStyle} /></div>)}
            </div>
            <div style={{ display:"flex", gap:"12px", marginTop:"28px" }}>
              <button style={{ display:"flex", alignItems:"center", gap:"6px", padding:"11px 22px", borderRadius:"100px", border:"2px solid #DA007C", background:"white", color:"#DA007C", fontSize:"14px", fontWeight:"600", cursor:"pointer" }}
                onMouseEnter={e=>{ e.currentTarget.style.background="#FFF0F8"; }} onMouseLeave={e=>{ e.currentTarget.style.background="white"; }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#DA007C" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Agregar evento
              </button>
              <button onClick={() => setModalOpen(false)} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"11px 22px", borderRadius:"100px", border:"2px solid #DA007C", background:"white", color:"#DA007C", fontSize:"14px", fontWeight:"600", cursor:"pointer" }}
                onMouseEnter={e=>{ e.currentTarget.style.background="#FFF0F8"; }} onMouseLeave={e=>{ e.currentTarget.style.background="white"; }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
